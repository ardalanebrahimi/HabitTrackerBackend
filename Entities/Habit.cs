...
public class Habit
{
    ...
    public int CopyCount { get; set; } = 0; // Track how many times the habit has been copied

    public List<HabitLog> Logs { get; set; } = new List<HabitLog>();

    public Habit CopyHabit()
    {
        return new Habit
        {
            Name = this.Name,
            ...
            AllowedGaps = this.AllowedGaps,
            StartDate = this.StartDate,
            UserId = this.UserId,
            IsArchived = this.IsArchived,
            IsPrivate = this.IsPrivate,
            CopyCount = this.CopyCount + 1
        };
    }
}
